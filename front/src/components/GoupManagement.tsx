...
const AddGroupModal: React.FC<AddGroupModalProps> = ({
    ...
    const handleSubmit = async () => {
        ...
        setState(prev => ({ ...prev, isSubmitting: true, validationError: null }));
                    ...
                    {error && (
                        <div style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                ...
                                <span style={{ marginLeft: '0.5rem', fontSize: '0.875rem' }}>{error}</span>
                            </div>
                        </div>
                    )}
                ...
                <div style={{
                    ...
                    <button
                        ...
                        style={{
                            ...
                            backgroundColor: 'white',
                            color: colors.text.secondary,
                            cursor: state.isSubmitting || isLoading ? 'not-allowed' : 'pointer',
                    ...
                    <button
                        ...
                        disabled={state.isSubmitting || isLoading || !!state.validationError || !state.groupName.trim()}
                        style={{
                            ...
                            cursor: (state.isSubmitting || isLoading || !!state.validationError || !state.groupName.trim()) ? 'not-allowed' : 'pointer',
                            opacity: (state.isSubmitting || isLoading || !!state.validationError || !state.groupName.trim()) ? 0.5 : 1,
                            transition: 'all 0.2s',
                            display: 'flex',
                            alignItems: 'center'
                        }}
                        onMouseEnter={(e) => {
                            if (!state.isSubmitting && !isLoading && !state.validationError && state.groupName.trim()) {
                        ...
                        <div style={{ display: 'grid', gap: '0.75rem' }}>
                            {groups.map((group, index) => (
                                ...
                                >
                                    ...
                                    <span style={{ color: '#111827', fontWeight: '500' }}>{group}</span>
                                </div>
                            ))}
                        </div>
                    ) : (
